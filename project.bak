        ...
        if matrix[rowIndex, colIndex] != 0:
            ...
            for currentRow in range(rowIndex + 1, matrix.shape[0]):
                if matrix[currentRow, colIndex] > 0:
                    currentLMatrix[currentRow, :] = currentLMatrix[currentRow, :] - currentLMatrix[rowIndex, :] * (
                    matrix[currentRow, colIndex] / matrix[rowIndex, colIndex])
                else:
                    currentLMatrix[currentRow, :] = currentLMatrix[currentRow, :] + currentLMatrix[rowIndex, :] * (
                    matrix[currentRow, colIndex] / matrix[rowIndex, colIndex])
                currentLMatrix[currentRow, colIndex] = -currentLMatrix[
                    currentRow, colIndex]  # Changing the sign of the item so that we don't need to take the inverse later.
...
def power_method(matrix, initialEV, error, n):
    ...
    ev = np.matrix(ev)
    vectorY = multiplyMatrices(ev, matrix)
    ev = normalize(vectorY)
    ev = np.matrix(ev)
    evTransposed = ev.getT()
    numeratorMatrix = multiplyMatrices(evTransposed, matrix)
    numeratorMatrix = multiplyMatrices(numeratorMatrix, ev)
    numerator = numeratorMatrix[0, 0]
    denominatorMatrix = multiplyMatrices(evTransposed, ev)
